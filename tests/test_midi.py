...
def test_quantize_keeps_chord_markers():
    """The engraving MIDI still carries the harmony added to the listening MIDI."""
    grid = BeatGrid(
        bpm=120,
        beats_per_bar=4,
        first_downbeat=0.0,
    ...
    midi = notes_to_midi(
        _sample_notes(), beat_grid=grid, chords=chords, quantize=True
    )
