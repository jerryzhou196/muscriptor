...
- **The muscriptor server** must allow the Vercel domain, including the
  ...
  preflight). Set its comma-separated `MUSCRIPTOR_ALLOWED_ORIGINS` environment
  variable, for example
  `MUSCRIPTOR_ALLOWED_ORIGINS=https://muscriptor.vercel.app`. Its base URL must
  be a browser-trusted HTTPS origin; a Cloudflare named tunnel in front of the
  loopback-only Vast.ai service is one way to provide that.
- **The chord service** uses a strict CSV allowlist (`ALLOWED_ORIGINS`, never
  `*`), so the Vercel domain has to be in it. Hugging Face provides its TLS.
