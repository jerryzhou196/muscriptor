...
# An optional shared secret a server-side proxy sends along, so the Space can
# be closed to that caller without standing up full user authentication. A
# static Vite frontend cannot keep this value secret, so enforcement is off by
# default and must only be enabled when such a proxy is in front of the Space.
