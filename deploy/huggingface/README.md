...
| `ENFORCE_EDGE_SHARED_SECRET` | `false` | When true, `/analyze` requires the shared secret header. Only usable behind a server-side proxy — a static frontend cannot hold a secret (see the abuse runbook). |
...
5. `ENFORCE_EDGE_SHARED_SECRET=true` with `EDGE_SHARED_SECRET` set — but only
   where something server-side can hold the secret. **A static frontend
   cannot.** Vite inlines `VITE_*` variables into the bundle it ships, so a
   secret configured there is readable by anyone who opens the page, and
   turning this on against a browser client locks out only the honest half of
   the traffic. It is the switch that actually stops a determined caller —
   IP-based limits are only as good as `X-Forwarded-For` — but it needs a
   proxy in front (a Vercel Route Handler, say) that keeps the secret
   server-side and forwards the upload.
6. Set the Space to **private** in its settings, or pause it, and let the
   ...
   Private is not a way to keep serving a browser frontend, for the same
   reason: a private Space answers only requests carrying an
   `Authorization: Bearer <token>` header, and a static bundle has nowhere to
   keep that token either. Private means "off, except to me and to servers I
   control".
